...
// Constants
var CMDB_API = {
  table: 'notifyconfig', // 表名
  showCloumn: '/web/people/updatePeopleShowColumn', // 个人展示列
  queryDictionary: '/web/queryDictionary', // 数据字典
  add: '/web/notify/adds', // 增
  delete: '/web/deleteNotifyconfigs', // 删
  // update: '/web/updateNotifylabel', // 改
  column: '/web/notifyconfig/column', // 查询条件
  query: '/web/notifyconfig/query', // 查
  export: '/web/excel/download/notifyconfig', // export
  import: '/web/excel/upload/notifyconfig' // import
};
META['code'] = { label: 'Code', required: true, maxLength: 200 };
META['name'] = { label: '标签通知名', maxLength: 200 };
META['notify'] = { label: '通知方式', maxLength: 200 };
META['configdatacode'] = { label: '配置项数据code', maxLength: 200 };
META['configname'] = { label: '配置项', maxLength: 200 };
...
var createData = function (item) {
  ...
  var init = function(){
    ...
    if (!isEdit) {
      ...
      var _dataJson = {data: [{paths: ["notifyconfig"], tableName: "notifyconfig", columnName: "code", values: []}]};
      ...
      WUI.ajax({
        ...
        $.each(resp.list, function(i, e){
          ...
          _dataFrom['configdataid'] = e.configdataid;
          _dataFrom['configname'] = nameToTable(e.configname);
          _dataFrom['configdatacode'] = e.configdatacode;
          _dataTo['people_id'] = e.people_id;
          _dataTo['people_name'] = e.people_name;

          var _hasThisLabel = false;
          $.each(relationConfig1, function(j, f){
            if (f['configdataid'] == _dataFrom['configdataid']) {
              _hasThisLabel = true;
...
submitData = function(item){
  ...
  if (relationConfig1.length > 0) {
    _submitData['configList'] = relationConfig1;
  } else {
    WUI.alert.create({
...
var QueryDataConfig1 = function (options) {
  ...
  WUI.ajax({
    ...
    $.each($('.relation-from .data-table tbody tr'), function(i, e){
      ...
      $.each(relationConfig1, function(j, f){
        if (_id == f['configdataid'] && _code == f['configdatacode']) {
    ...
    $('.relation-from .data-table input[type=checkbox]').click(function(){
      var _this = $(this);
      var _thisJson = {};
      _thisJson['configdataid'] = _this.attr('data-id');
      _thisJson['configname'] = nameToTable(_this.parent().parent().find('td').eq(1).text());
      _thisJson['configdatacode'] = _this.parent().parent().find('td').eq(2).text();
      if (_this.is(':checked')) {
        relationConfig1.push(_thisJson);
      } else {
        $.each(relationConfig1, function(i, e){
          if (e['configdataid'] == _thisJson['configdataid'] && e['configdatacode'] == _thisJson['configdatacode']) {
  ...
  $.each(data, function(i, e){
    ...
    $.each($('.relation-result-from .relation-pop p'), function(j, f){
      if ($(this).text() == tableToName(e['configname'])) {
        _hasThisP = true;
        $('.relation-result-from .relation-pop ul').eq(j).append('<li>'+ e['configdatacode'] +'</li>');
    ...
    if (!_hasThisP) {
      $('.relation-result-from .relation-pop').append('<p class="active"><span></span>'+ tableToName(e['configname']) +'</p>');
      $('.relation-result-from .relation-pop').append('<ul><li>'+ e['configdatacode'] +'</li></ul>');
    }
  });
};
